...
const isOriginAllowed = (origin) => {
  ...
  if (origin === 'https://freedomdiagnosticstesting.com') return true;
  if (origin === 'https://koveralabs.com') return true;
...
// New format: "Product: GHK-CU 50mg   Purity: 99%"
...
function parseFields(text, filename) {
  ...
  // Lot: CP-style code covers new format; old format has "N/A" which we leave null
  const lot = (text.match(LOT_RE) || [])[0] || null;

  // Detect old format by "Product Lot" header (no colon between them)
  let product = null;
  if (/\bProduct\s+Lot\b/i.test(text)) {
    // Old format: value line is "{product} N/A" or "{product} {CP-lot}"
...
app.post('/api/bulk-import-kovera', (req, res) => {
  const { rows } = req.body || {};
...
app.get('/api/bulk-import-results/:token', (req, res) => {
  const results = pendingImports.get(req.params.token);
  if (!results) return res.status(404).json({ error: 'Import token not found or expired.' });
  pendingImports.delete(req.params.token);
  res.json({ results });
});
